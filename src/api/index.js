...
async function getData(url) {
    ...
    try {
        ...
        if (axios.isCancel(err)) {
            return require('@/static/data.json');
        }
    } finally {
...
const getShotImagesByMovieId = async id => {
    return await getData(
        `${API_URL_CATEGORY_FILMS}/${id}${API_PARAM_SHOT_IMAGES}${API_PARAM_PAGE}1`,
    );
};
...
export const getVideosFromYoutube = async(movieTitle, releaseYear) => {
    try {
        const videosAll = await YouTubeClient.get(
            `search?part=snippet&q=Трейлер ${movieTitle} ${releaseYear}&type=video&videoDuration=short&maxResults=1&key=AIzaSyAQdRphaSUuFoKjC7O_UF4IK5d1pcEw5QU`,
        );
